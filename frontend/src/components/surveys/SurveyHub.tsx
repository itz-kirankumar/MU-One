...
export function SurveyHub() {
  ...
  return <div className="space-y-7">
    ...
    <div className="flex items-center justify-between gap-3"><div role="tablist" aria-label="Survey lists" className="flex gap-1 rounded-xl border border-[#292929] bg-[#111] p-1">{[false, true].map(value => { const label = value ? 'My surveys' : 'Explore surveys'; return <button ref={value ? mineTabRef : exploreTabRef} key={String(value)} id={`survey-tab-${value ? 'mine' : 'explore'}`} role="tab" tabIndex={mine === value ? 0 : -1} aria-selected={mine === value} aria-controls="survey-feed-panel" onClick={() => setMine(value)} onKeyDown={handleTabKey} className={`min-h-11 rounded-lg px-4 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f7d344] ${mine === value ? 'bg-[#292929] text-white' : 'text-gray-400'}`}>{label}</button>; })}</div><button onClick={() => setRevision(r => r + 1)} aria-label="Refresh surveys" title="Refresh surveys" className="min-h-11 min-w-11 rounded-lg border border-[#333] p-2.5 text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f7d344]"><RefreshCw size={16} aria-hidden="true" /></button></div>
