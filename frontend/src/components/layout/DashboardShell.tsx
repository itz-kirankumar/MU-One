...
import { AlertTriangle, ExternalLink, Mic, Calendar } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0A0A0A]">
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        ...
        >
          <div className={`mx-auto px-4 py-6 ${
            activeTab === 'email' ? 'max-w-[1600px] w-full h-full' : 
            activeTab === 'dashboard' ? 'max-w-[1600px] w-full' : 
            'max-w-5xl w-full'
          }`}>
            {activeTab === 'dashboard' ? (
              <div className="flex flex-col xl:flex-row gap-6">
                ...
                <div className="flex-1 min-w-0 space-y-6">
                  ...
                  {/* Row 5: Mail */}
                  <ImportantMail onOpenEmailCenter={() => setActiveTab('email')} />
                </div>
                
                {/* Right Sidebar: Weekly Focus + Google Tasks */}
                <div className="w-full xl:w-[350px] 2xl:w-[400px] flex-shrink-0 space-y-6 flex flex-col">
                  <WeeklyFocus />
                  <GoogleTasksList />
                </div>
              </div>
      ...
      <button
        ...
        className="fixed bottom-[84px] right-6 z-40 flex h-10 w-10 items-center justify-center rounded-full bg-[#161616] text-gray-300 shadow-xl hover:scale-105 active:scale-95 transition-all cursor-pointer border border-[#333] hover:text-white"
      >
